import numpy as np  # Thư viện xử lý mảng đa chiều
import matplotlib.pyplot as plt  # Thư viện vẽ đồ thị và hình ảnh
import cv2  # Thư viện xử lý ảnh
from tkinter import filedialog  # Thư viện để chọn file
import tkinter as tk  # Thư viện giao diện đồ họa Tkinter

# Hàm để chọn và mở file ảnh
def open_image():
    root = tk.Tk()
    root.withdraw()  # Ẩn cửa sổ gốc
    file_path = filedialog.askopenfilename()  # Chọn file ảnh
    if file_path:
        image = cv2.imread(file_path, 0)  # Đọc file ảnh
        return image
    else:
        return None
# Định nghĩa hàm biến đổi DFT 1D
def DFT1D(img):
    U = len(img) #Độ dài ảnh
    outarry = np.zeros(U, dtype=complex)  # Mảng chứa kết quả DFT
    for m in range(U):
        sum = 0.0
        for n in range(U):
            e = np.exp(-1j * 2 * np.pi * m * n / U)  # Phần tử phổ
            sum += img[n] * e  # Tính tổng = Phần tử của anh * e
        outarry[m] = sum  # Lưu kết quả DFT vào mảng
    return outarry

# Định nghĩa hàm biến đổi ngược DFT 1D
def IDFT1D(img):
    U = len(img)  # Kích thước ảnh
    outarry = np.zeros(U, dtype=complex)  # Mảng chứa kết quả IDFT
    for n in range(U):
        sum = 0.0
        for m in range(U):
            e = np.exp(1j * 2 * np.pi * m * n / U)  # Phần tử phổ
            sum += img[m] * e  # Tính tổng
        pixel = sum / U  # Chia cho kích thước ảnh
        outarry[n] = pixel  # Lưu kết quả IDFT vào mảng
    return outarry

# Định nghĩa hàm lọc thông thấp Ideals
def lowPass_Ideals(D0, U, V):
    # H là bộ lọc
    H = np.zeros((U, V))  # Khởi tạo ma trận bộ lọc
    D = np.zeros((U, V))  # Khởi tạo ma trận khoảng cách
    U0 = int(U / 2)
    V0 = int(V / 2)
    # Tính khoảng cách
    for u in range(U):
        for v in range(V):
            u2 = np.power(u, 2)  # Bình phương u
            v2 = np.power(v, 2)  # Bình phương v
            D[u, v] = np.sqrt(u2 + v2)  # Tính khoảng cách Euclid
    # Tính bộ lọc
    for u in range(U):
        for v in range(V):
...
if __name__ == "__main__":
    ...
    if image is None:
        print("Không có file ảnh được chọn!")
    else:
        # Tiếp tục xử lý ảnh và hiển thị kết quả
        M, N = np.shape(image)  # Chiều x và y của ảnh
